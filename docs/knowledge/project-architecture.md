...
`run_daily_analysis.ps1` 和 `run_daily_analysis.sh` 负责编排，`.github/workflows/stock-selection.yml` 在自托管 Windows runner 上触发。`.cache` 保存大量持久化行情和财务数据，`选股结果`、`板块观察` 与 `logs` 保存运行产物。
...
封装外部数据源。每个适配器实现统一接口，负责请求、源字段映射和原始响应，不负责策略评分。适配器必须返回来源、抓取时间和可用的公告/修订时间。

### `storage`

负责原始文件、SQLite 目录、事务、哈希、断点状态和时点查询。策略不能绕过这一层直接挑选“最新文件”。
...
实现价值线适用性、基本面门槛、主线分层和三浪三筛选。策略通过接口获取数据，不知道底层是 AkShare、Baostock、SQLite 还是文件。

### `pipelines`

创建运行上下文、调用各模块、写运行清单、执行覆盖率和日期门控。历史回填、每日增量、生产日报和离线验证分别有明确入口。

### `reporting`

只格式化已经通过门控的结构化结果。报告模块不重新选股、不自行寻找最新数据，也不修改策略结论。
...
新数据源应实现适配器契约，返回标准股票代码、源记录标识、原始内容、公告/修订时间和抓取时间。必须提供字段映射测试、错误分类、限速策略和至少一个本地固定样本。不得在策略模块中直接调用第三方接口。

## 8. 添加新指标或策略

新指标先作为纯函数加入 `indicators`，使用固定输入测试。新策略加入 `strategies`，显式声明所需数据、观察日规则、硬门槛、缺失处理和输出字段。策略结果必须进入运行清单后才能被报告消费。
