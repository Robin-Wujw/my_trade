...
当前持久化边界总体合理：`raw` 保存可复用事实数据，`derived` 保存候选、Formula33阶段和回测结果，`var/backtests` 保留 CSV/Markdown 作为人工审阅入口。候选和成交表保留可直接查询的买卖依据字段：
...
这样 2024-09-24 至今的候选、依据、IMA/网页对照、多周期强势成长依据和最终成交可以在 DuckDB 中按日期、版本和 run_id 追溯；CSV 仍作为可读产物，不再承担唯一事实源。
