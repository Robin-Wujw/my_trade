...
组合回测使用收盘后形成的候选和 Formula33 状态，并从下一交易日生效。主交易名额默认 3 只，全部持仓标的硬上限 5 只，日常目标约 3 只；所有行情下左侧价值标的最多 1 只，左+右混合仓也计入这个左侧名额。若已有多只左侧标的，只保留最优一只，其余按组合限额退出。单票总仓位不超过 62.5%，组合总仓位不超过 100%。
